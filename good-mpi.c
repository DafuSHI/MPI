...
int main(int argc, char *argv[])
{
  ...
  int nl = (int)((2*MAXX+1)/nbslaves)+1;  //Mei ge slaves yao suan de liang  There is error, if nl is integer no need to add 1
...
*/
int wait_slaves = nbslaves ;
while(wait_slaves > 0){
  printf("%d\n",wait_slaves);
MPI_Recv(&res, nl, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
k = status.MPI_SOURCE;
tag = status.MPI_TAG;
for(i = 0; i < nl; i++ )
for(j = -MAXY; j <= MAXY; j++) {
      cases[i + nl * (k-1) ][ j + MAXY] = res[ i ][j + MAXY];
      }
//if(tag % np == 0 || tag == 2*MAXX)
  ...
  printf("This is from %d\n",k);
    ...
    // have big problmes 23/10/2016
    /* On est l'un des fils */
    double x, y;
    int i=-MAXX+(rank-1)*nl, j, res[nl][2*MAXY+1], rc;

    //for(i = -MAXX; i <= MAXX; i++) {
    for(i = -MAXX+(rank-1)*nl; i < -MAXX+(rank)*nl; i++) {
      if (i == MAXX+1) break;
      for(j = -MAXY; j <= MAXY; j++) {
      	x = 2 * i / (double)MAXX;
      	y = 1.5 * j / (double)MAXY;
      	res[i+MAXX-(rank-1)*nl][MAXY+j] = mandel(x, y); //this is for yin she
      ...
      {
        for(j = -MAXY; j <= MAXY; j++){
          printf("%d ",res[i+MAXX-(rank-1)*nl][MAXY+j]);
          if(j == MAXY) printf("\n"); 
        }
      }
      MPI_Send(&res, nl , MPI_INT, 0, rank, MPI_COMM_WORLD); //this line should be outside 
  }
